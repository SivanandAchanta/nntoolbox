function [gU,Ebb] = bp_cl_v2(U,Eb,ym,X,K,nin,nout,f,sl)
...
if mod(K,2) == 0
    ...
    for i = 1:nout % process filter by filter
        ...
        for j = (1 + Kby2):(sl + Kby2) % process by sliding window across signal
            Ebb(j-Kby2:j+Kby2-1,:) = Ebb(j-Kby2:j+Kby2-1,:) + U(:,(i-1)*nin+1:i*nin)*Eb(j-Kby2,i); % width = K filters
            gU(:,(i-1)*nin+1:i*nin) = gU(:,(i-1)*nin+1:i*nin) + X(j-Kby2:j+Kby2-1,:)*Eb(j-Kby2,i);
        end
    end
    Ebb = Ebb((1 + Kby2):(sl + Kby2),:);
else
    ...
    for i = 1:nout % process filter by filter
        ...
        for j = (1 + Kby2m1):(sl + Kby2m1) % process by sliding window across signal
            Ebb(j-Kby2m1:j+Kby2m1,:) = Ebb(j-Kby2m1:j+Kby2m1,:) + U(:,(i-1)*nin+1:i*nin)*Eb(j-Kby2m1,i); % width = K filters
