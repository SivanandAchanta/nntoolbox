...
def evaluate(encoder, decoder, pfnet, prompts, phn2id, id2phn, vocab_size, use_cuda, criterion, op_dim, teacher_force, opt, mo1, so1, nml_vec1, mo2, so2, nml_vec2):
    ...
    for k in prompts:
        ...
        if teacher_force:
            ...
            # AlwaysSamplingg: Feed the "predicted" target as the next input
            for di in range(target_length):
                decoder_output1, decoder_output2, decoder_h1, decoder_c1, decoder_h2, decoder_c2, decoder_h3, decoder_c3, decoder_attention = decoder(
                    decoder_input, decoder_h1, decoder_c1, decoder_h2, decoder_c2, decoder_h3, decoder_c3, encoder_outputs)
                loss += criterion(decoder_output1, target_variable[di])
        ...
        loss2 = loss2 + val_loss + (val_loss1.data[0] / (opt.r*target_length))
        ...
        y = y / np.max(np.abs(y))
        sf.write( '../../synth_wav/' + k + 'tf_' + str(teacher_force) + '.wav', y, 16000)
...
def train(opt):
    ...
    for iter in range(1, opt.niter + 1):
        ...
        for j, k in enumerate(train_prompts):
            ...
            print_loss_total2 += (loss2.data[0] / r*target_length)
            plot_loss_total2 += (loss2.data[0] / r*target_length)
            ...
            if (j+1) % print_every == 0:
                ...
                if avg_val_loss_pf1 < best_val_loss:
                    best_val_loss = avg_val_loss_pf1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', required=False, type=float, default=0.00003, help='0.0003')
    ...
    parser.add_argument('--r', required=False, type=int, default=3, help='2| 3| 4| 5')
    ...
    parser.add_argument('--train_flag', required=False, type=bool, default=False, help='True | Fasle')
