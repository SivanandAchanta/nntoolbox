function [ol] = fpav_gpu(X,Wt,b,nl,fl,nh,wtl,btl,a_tanh,b_tanh,Nin)
...
Xm = [gpuArray(ones(Nin,1)) X]; % ( Nin x (din+1))
rWt = reshape(Wt(1,wtl(1):wtl(2)-1),nl(2),nl(1))';

pac = Xm*[b(1,btl(1):btl(2)-1);rWt];
ac = get_actf(fl(1),pac,a_tanh,b_tanh);
