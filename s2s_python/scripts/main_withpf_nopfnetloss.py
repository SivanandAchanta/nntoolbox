...
def train(opt):
    ...
    if opt.load_wts:
        load_model_name_pfx = '../../wt/s2s_enc_blstm_dec_lstm3l_pfnet_blstm1L_nopfnetloss__hs1250_hs2500_pfnet_hs1250_r3_lr0.0003_b10.9_b20.99_dp0.5_gc0.0_wtinitdefault_init_lw0_ef0_rf1_'
        load_model_name_sfx = '_epoch_2999_15.pth'
    ...
    for iter in range(1, opt.niter + 1):
        ...
        for j, k in enumerate(train_prompts):
            ...
            if (j+1) % print_every == 0:
                ...
                avg_val_loss_total_as, avg_val_loss_as, decoder_attentions_pf = evaluate(encoder.eval(), decoder.eval(), pfnet.eval(), val_prompts, phn2id, id2phn, vocab_size, use_cuda, criterion, op_dim, tf, opt, mo1, so1, nml_vec1, mo2, so2, nml_vec2)
                print('%d %0.4f %0.4f' % (iter, avg_val_loss_total_as, avg_val_loss_as))
                logging.debug('Epoch: ' + str(iter) + ' Update: ' + str(iter*len(train_prompts) - len(train_prompts) + j) + ' Avg Val Loss Total: ' + str(avg_val_loss_total_as))

                if avg_val_loss_total_as < best_val_loss:
                    best_val_loss = avg_val_loss_total_as
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', required=False, type=float, default=0.0000003, help='0.0003')
    ...
    parser.add_argument('--r', required=False, type=int, default=4, help='2| 3| 4| 5')
    ...
    # Additional flags
    parser.add_argument('--load_wts', required=False, type=bool, default=True, help='True | False')
